...
"""
VENTANA AYUDA
Pulsa uno de los siguientes botones para ir modificando el color de la figura

r: aumenta el valor del color rojo

g: aumenta el valor del color verde

b: aumenta el valor del color azul
...
for key,frame in stream:
    ...
    if debug:
        ...
        cv.drawContours(frame,[c.astype(int) for c in good], -1, (255,0,0), 3, lineType)
        ...
        # mostramos un objeto 3D virtual en colores
    cv.drawContours(frame,[ htrans(M,cube).astype(int) for M in poses ], -1, (blue,green,red), 3, lineType)
